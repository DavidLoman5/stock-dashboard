...
- [ ] **🔴 出場規則與排序權重要不要動？走前驗證已跑完，但兩個指標指向不同答案（2026-08-19）**
  backtest v2.2 樣本外（n=240/格）顯示**現行組合 `chip+t | prod` 的 alpha 為負（−0.29%）**，
  同一排序換任何一種出場都比它好（hold20 +0.22／stop +1.06／stopTrail +0.82）；
  排序方面技術權重越高 alpha 越差（chip 單獨最好 +1.67）。方向與 lessons.md 的
  「tech25+ alpha −1.66%」「外資連2賣已無統計優勢」「跌破月線 avgRet −3.01%」三條觀察一致。
  **卡住的地方**：`backtest.ps1` 表頭與頁面卡都明載「出場規則之間的超額不可直接比較」
  （提早出場者持有天數短、且未計資金再利用），要以**勝率**為主；而按勝率排，`hold20` 全面最佳、
  `stop` 反而輸給 `prod`。alpha 與勝率選出不同的贏家，鐵律的「樣本外顯著優於現行」因此不成立，
  所以本次**維持現行規則未動**。
  三個選項：
  1. 以 alpha 為準 → 排序改純 chip、出場改 `stop`（跌破月線停損，拿掉外資連2賣與移動停利）
  2. 以勝率為準 → 出場改 `hold20`（完全不提早出場、持有滿 20 日），排序仍可考慮降低技術權重
  3. 不動，再累積一個月樣本後重跑（保守；但現行組合的樣本外 alpha 是負的）
  無論選哪個，都要照鐵律「每月最多調一次、記錄於本檔」，並在 `index.html` 的 logicCard 同步。

  **2026-08-19 使用者決定：選項 3——先不動，累積一個月後重跑。**
  理由：判級前瞻驗證（`byStance`）與基本面數字都是這次才進管線的，下個月重跑時會多兩個
  彼此獨立的證據來源（`byStance` 的 monotonicity 走勢、`byFund`/`byYoy` 在有 MoM/累計 YoY
  之後的分組），比現在只靠一次網格、而且兩個指標互相矛盾的情況可靠得多。
  **已知代價：這一個月仍在使用一個樣本外 alpha 為負的組合。**
  下次重跑（約 2026-09 中）要看的三件事：
  1. `chip` 單獨排序是否仍勝過 `chip+t`（技術權重到底該不該降）
  2. `prod` 出場是否仍在同排序下墊底（若是，改出場的證據就跨了兩個月）
  3. `byStance` 的 monotonicity 有沒有從 47 往上動——那是判級 v3 修好凍結之後的第一份讀數
...
查證後發現四個結構性問題，其中兩個是缺陷而非調參。

**① 基本面那一面從來沒有素材（根因）**
`update-holdings.ps1` 的 `CodeContext()` 只產出價格／均線／法人／融資／除息——**一個基本面數字都沒有**。
AI 每天被要求寫 `fund` 段落卻無料可用，於是退化成大盤敘事（實例：`0050.fund` =
「今日下跌主因台積電領跌拖累大盤（詳見大盤『市場風向』區）」）。這是管線缺料，改 prompt 治不好。
`screen.ps1` 其實每天都抓了全市場 PE/PB/殖利率/月營收，只是沒接到持股這條線。

- 兩個抓取收編進 `lib/feed.ps1`：`Get-FeedValuation`（pe/pb/dy）、`Get-FeedRevenue`（name/ind/yoy/mom/yoyCum）。
  原本是 `screen.ps1` 裡四個 inline `GetJson`，其中月營收兩個端點用**位置索引**讀物件回應（`$p[9].Value`）——
  違反「解析也要在 feed 層」的規則。改為依欄位名讀取，`CheckRevCols` 由 `Test-FeedRowFields` 取代（會指名消失的欄位）。
- **新解析 `營業收入-上月比較增減(%)`（MoM）與 `累計營業收入-前期比較增減(%)`（累計 YoY）**，一直在同一份回應裡沒被讀。
  單月 YoY 分不出「成長」與「成長在減速」：2454 單月 +12.2% 但累計 +0.8%、MoM −16.4%；3661 單月 +181.8% 但累計 −13.8%。
- `CodeContext()` 擴充 `yoy/yoyCum/mom/pe/pb/dy/ind`，三份 context 檔自動同步。**ETF 一律 null 不是 0**（0 會被讀成「本益比 0」）。

**② 判級會無限期凍結（bug，非調參）**
`lib/stance.ps1` 的兩日確認寫成「連續兩日 raw **完全相同**」才換級。讀數在相鄰級別間擺動就永遠湊不齊。
實測 408 筆中 190 筆（47%）處於「新讀數被壓住」，最長凍結 9 個交易日。
代號 8150 在 8/14–8/18 原始讀數 cut／cutwatch／cut，畫面卻一直是「加碼觀察」，同期股價 99.0 → 89.1。
- 改為 v3：新讀數一律採用，除非「只差一格」且「昨日讀數與現行級別一致」（只吸收孤立的一日跳動）；
  差 2 格以上立即生效；連續兩日不同必定生效。全樣本重放：被壓住 40%→17%、最長凍結 8 天→**1 天**、≥3 天的凍結區間 18 段→**0 段**。
- `stance-log.json` 每列加 `v`（引擎版本）與 `idx`（當日加權指數收盤），歸因**只在同版本內比較**。

**③ 判級準確度從來沒有被量測**
`eval-report.json` 有 byReason/byLight/byInd/byAiTag/byChip/byTech/byFund/byYoy，就是沒有判級。
`evaluate.ps1` 其實有一段 `stanceForward`，但它要求每檔 20 列以後的前瞻資料，而帶 `raw` 的列每檔只有約 14 列——
**迴圈一次都沒跑過**，區塊靜靜地從未出現在報告裡。
- 改寫為 `Get-StanceAttribution`（放 `lib/stance-log.ps1`，純函式、可用 fixture 測）：5/10/20 日前瞻、
  alpha 基準為當日全樣本等權均值、勝率、以及頭條指標 **monotonicity**（六級排序有幾對成立；100=完全照順序、50=擲硬幣、0=完全相反）。
- **首次量測結果：fwd5 monotonicity 47、fwd10 40**——都低於 50，即目前的六級排序不帶前瞻資訊。
  fwd5 各級 alpha：加碼 +0.11／加碼觀察 −0.73／續抱 +1.32／減碼觀察 +2.80／減碼 +0.57／清倉 −0.27。
  樣本限制：僅 23 個交易日、逐日列高度重疊（非獨立樣本）、universe 是使用者持股。**不足以據此指定新門檻，只足以證明現行排序沒有效力。**

**④ 紅燈沒有真的擋下進場**
`$light` 只調權重並記在列上，`$top5` 照樣寫進 picks-log。而紅燈組 n=16、勝率 19%、alpha −2.21%，
是 lessons.md 裡唯一跨過 10 筆門檻且連續四週方向一致的教訓。
- 新增 `Test-RegimeEntryGate`（`lib/score.ps1`）：紅燈日不寫入 picks-log；頁面仍照常排名與顯示。

**評分引擎：三面對稱化（重構，數值不變）**
三面改為統一形狀 `@{Raw; Score(0-100); Signals[]; Drop; Reason}`，新增唯一合成器 `Get-CompositeScore`。
權重刻意取原本三面的滿分比例（股票 0.40/0.30/0.30；ETF 40/70、30/70、fund=0），
**因此合成分數與過去的「40+30+30 直接相加」在數值上完全相同**——這正是讓重構可被驗證的性質：
重跑 `screen.ps1`，五檔選股與三面分數逐項一致、`picks-log.json` byte-identical。
ETF 由 70 分制改為同樣的 0–100（`(chip+tech)×100/70`，純比例縮放，排序不變），首次可與個股直接比較。
`Signals[]` 同時就是頁面數字列的資料來源。`.Raw` 保持原尺度寫進 picks-log，歸因的 chip30-37/tech25+ 等分組不受影響。

**backtest v2.2 走前驗證（200 面板日、eval 120 日、樣本外 n=240/格）**

| ranking | hold20 | stop | prod(現行) | stopTrail |
|---|---|---|---|---|
| chip | +1.05 | **+1.67** | +0.50 | +1.18 |
| chip+0.5t | −0.06 | +0.94 | +0.14 | +0.53 |
| chip+t（現行） | +0.22 | +1.06 | **−0.29** | +0.82 |
| chip+2t | 0.00 | +0.53 | +0.14 | +0.25 |
| chip+4t | −0.79 | −0.46 | −0.22 | −0.84 |
| tech | −2.54 | −0.87 | −0.10 | −1.23 |

兩個一致的訊號：**現行組合（chip+t | prod）樣本外 alpha 為負（−0.29%），且同一排序下其他三種出場全都比它好**；
**技術權重越高 alpha 越差**（chip > chip+0.5t ≈ chip+t > chip+2t > chip+4t > tech），與 lessons.md 的 tech25+ 觀察同向。
- **本次未改任何權重或出場規則。** 理由：`backtest.ps1` 表頭與頁面卡都載明「出場規則之間的超額不可直接比較」
  （提早出場者持有天數短、未計資金再利用），而改以勝率比較時 `hold20` 最佳、`stop` 反而不如 `prod`——
  兩個指標指向不同的候選。鐵律是「樣本外顯著優於現行＋空頭區不劣化才動手，每月最多一次」，
  在指標互相矛盾時「顯著優於」並不成立。**待使用者拍板要以 alpha 還是勝率為準（見「待決策」）。**
- 基本面因子**仍無法回測**（無營收/PE 歷史面板，`backtest.ps1:20` 自述）。因此 MoM／累計 YoY 只上頁顯示、不計分。

**市場風向獨立（使用者要求的「獨立一欄」）**
- 資料層：從 `holdings-notes.json` 的假代號 `_market` 抽成獨立檔 `market-notes.json` ＋ 獨立區塊 `window.MARKET`
  （`page-contract.json` 新增 `marketnotes` block 與 `marketFields` 政策，取代 `noteFields.marketPublic`）。
  寄生在個股筆記裡正是個股 `fund` 段落能順手拿大盤敘事的結構原因。
- 版面層：從「大盤行情」卡抽出，自成「🧭 市場風向」卡；大盤卡只留量化數字。
- 政策層：`marketFields.guest` = windLead/sox/mood/moodK；`ownerOnly` = wind/night（維持原本的可見範圍）。
  發佈閘門現在**同時**掃兩份 ownerOnly 清單，否則 `night` 會無人看管。
- `.gitignore` 補 `market-notes.json`；`publish.ps1` 加 splice；`run-daily.sh` 的 prev-recs 產生器改讀新檔，
  並把原本整段 `|| true` 的靜默失敗改成會印 WARN（格式一變 AI 就無聲失去昨日回顧，先前無任何告警）。

**其他**
- `rec` 依使用者指示**維持現狀**，一行未動（仍為 ownerOnly、仍可引用風向與投組）。這是刻意的例外。
- `sigFund` 廢除：基本面方向標籤改由 `Get-FundSignal` 計算，與技術／籌碼兩個標籤一致。
  先前只有它是 AI 自己寫的，與旁邊任何數字都沒有已定義的關係。`gnotes.py` 即使模型主動給了也不採用。
- 順手修掉兩個既有缺陷：`screen.ps1` 股票流動性硬編碼 `1e8`（未用 profile）；ETF 流動性檢查寫成
  `if($px.ContainsKey($c) -and ...)`，**不在 `$px` 的 ETF 會整個跳過流動性門檻**。
- `HOLDINGS_META` 新增 `_fund` 聯集對照表，guest 也能看到自己持股的基本面數字（交易所數字、無 AI 成本，
  與 `_prevStance` 同一個模式）。
- 測試：`tests.ps1` 新增 byStance 屬性測試（含「非空輸入必須產生報告」——擋住我在開發中踩到的
  PowerShell 變數**大小寫不敏感**陷阱：函式內 `$rows=@()` 會清空參數 `$Rows`，於是任何輸入都回 null）、
  v2/v3 確認規則重放對照、紅燈閘門、三面正規化與合成、`Test-FeedRowFields`；
  `server/test_server.py` 98 項；`tools/boot-check.js` 42 項（新增基本面數字列、ETF 空值、MARKET 缺席）。
